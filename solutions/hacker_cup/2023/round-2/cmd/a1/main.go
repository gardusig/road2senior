...
func countComponentSize(i int, j int, N int, M int, board []string, used [][]bool, availableBordersForComponent map[[2]int]bool) {
	used[i][j] = true
	for k := 0; k < 4; k++ {
		x := i + dx[k]
		y := j + dy[k]
		...
		if board[x][y] == 'W' {
			countComponentSize(x, y, N, M, board, used, availableBordersForComponent)
		} else if board[x][y] == '.' {
...
func solveTestCase(input *TestCaseInputData) string {
	...
	for i := 0; i < input.N; i++ {
		for j := 0; j < input.M; j++ {
			...
			countComponentSize(i, j, input.N, input.M, input.A, used, availableBordersForComponent)
